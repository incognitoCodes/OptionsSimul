...
def parse_strike(raw: str) -> float:
    # Moomoo: strikes like 207500 → 207.5
    s = str(raw)
    if s.isdigit():
...
def parse_moomoo_positions(df: pd.DataFrame) -> Tuple[List[Strategy], List[OptionLeg]]:
    ...
    for _, row in df.iterrows():
        ...
        # Skip spread header rows like NVDA251031P195/200; legs follow below
        ...
        if key:
            strategies.setdefault(key, Strategy(key, [], True, "current")).legs.append(leg)
        else:
    ...
    for s in strategies:
        ...
        if ticker:
            if ticker in s.tickers():
                tot += strategy_pl_curve(s, xs, ticker)
        else:
            for tk in s.tickers():
...
def breakevens(xs: np.ndarray, ys: np.ndarray) -> List[float]:
    ...
    for i in idx:
        ...
        y0, y1 = ys[i], ys[i+1]
        if y1 != y0:
            out.append(round(float(x0 - y0*(x1-x0)/(y1-y0)), 4))
    return out
...
def build_iron_butterfly(t, exp, center_k, put_wing, call_wing, qty, credit, tag="what-if") -> Strategy:
    return Strategy(f"{t} {exp} Iron Butterfly {put_wing:g}-{center_k:g}-{call_wing:g}", [
        OptionLeg(t, exp, "P", center_k, -abs(qty), abs(credit)/2.0),  # why: attach half credit here
...
if st:
    ...
    def get_available_strikes(provider: str, ticker: str, expiry: dt.date,
        ...
        """Return sorted unique strikes for the nearest available expiry (Yahoo implemented)."""
        strikes: List[float] = []
        try:
    ...
    def fetch_spot(provider: str, ticker: str, api_key: str = "", moomoo_cookie: str = "") -> Optional[float]:
        try:
            if provider == QuoteProvider.Yahoo and yf:
                ...
                try:
                    info = t.info or {}
                    v = _safe_float(info.get("regularMarketPrice") or info.get("currentPrice"))
                    if v and v > 0:
                        return v
                except Exception:
                    pass
            ...
            st.warning(f"Spot fetch error for {ticker}: {e}")
            return None
        ...
        try:
            if provider == QuoteProvider.Yahoo and yf:
                ...
                try:
                    ...
                    st.warning(f"Option chain fetch error {tk} {exp}: {e}")
                    return None
            ...
            st.warning(f"Option mid fetch error {ticker} {expiry} {kind}{strike}: {e}")
            return None
...
def st_app():
    ...
    if uploaded is not None:
        ...
        st.warning("Upload a CSV or load the sample to begin.")
        st.stop()
    ...
    for tk in tickers:
        ...
        spot_inputs[tk] = st.sidebar.number_input(f"{tk} spot", min_value=0.0,
                                                  value=float(st.session_state[f"spot_{tk}"]),
                                                  step=1.0, key=f"spot_{tk}")
        if st.sidebar.button(f"Fetch {tk} spot", key=f"btn_spot_{tk}"):
            ...
            if fetched:
                ...
                st.warning(f"{tk}: could not fetch spot from {provider}. Set manually or try another provider.")
    ...
    with st.expander("Builder"):
        ...
        with tabs[1]:
            ...
            if st.button("➕ Add Vertical"):
                ...
                if kind == "P" and ls > ss: ls, ss = ss, ls  # why: maintain credit vertical direction
    ...
    tk0 = tickers[0]
    xs = price_grid(spot_inputs[tk0], pct, steps)
    current = aggregate_pl_curves(strategies, xs, ticker=tk0)
    added   = aggregate_pl_curves(what_if, xs, ticker=tk0)
    comb    = current + added
    fig2 = go.Figure()
...
def _run_tests():
    _test_payoff_basic(); _test_parser_and_grouping()
...
def main():
    ...
    if st is None:
        ...
        print("Run: streamlit run option_pl_simulator.py")
        return
    st_app()
