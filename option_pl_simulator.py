...
# (shortened header to save space)
...
@dataclass
class OptionLeg:
    ticker:str; expiry:dt.date; kind:str; strike:float; qty:int; avg_cost:float; name:str=""; source_id:Optional[str]=None
    ...
    def label(self)->str:
        return f"{self.ticker} {self.expiry:%Y-%m-%d} {'L' if self.qty>0 else 'S'}{self.kind}{self.strike:g} x{abs(self.qty)} @ {self.avg_cost}"

@dataclass
class Strategy:
    name:str; legs:List[OptionLeg]=field(default_factory=list); enabled:bool=True; user_tag:str=""
    def tickers(self)->List[str]: return sorted({l.ticker for l in self.legs})
    def expiries(self)->List[dt.date]: return sorted({l.expiry for l in self.legs})
...
def parse_expiry(s:str)->dt.date: return dt.date(2000+int(s[:2]), int(s[2:4]), int(s[4:6]))
def parse_strike(raw:str)->float:
    s=str(raw)
    if s.isdigit(): return float(s)/1000.0
    try: return float(s)
    except ValueError: return float(re.sub(r"[^\d.]", "", s))
...
def parse_moomoo_positions(df:pd.DataFrame)->Tuple[List[Strategy], List[OptionLeg]]:
    ...
    strategies:Dict[str,Strategy]={}; legs:List[OptionLeg]=[]
    for _,row in df.iterrows():
        symbol=str(row["Symbol"]).strip(); name=str(row.get("Name","")).strip()
        qty=int(float(row["Quantity"])); avg=float(row.get("Average Cost",0.0))
        if SPREAD_SYMBOL_RE.match(symbol):
            if name and name not in strategies: strategies[name]=Strategy(name,[],True,"current")
            continue
        m=LEG_SYMBOL_RE.match(symbol.replace(" ","")) or (LEG_SYMBOL_RE.match(name.replace(" ","")) if name else None)
        if not m: continue
        tk=m.group("ticker"); expiry=parse_expiry(m.group("expiry")); kind=m.group("kind"); strike=parse_strike(m.group("strike"))
        leg=OptionLeg(tk,expiry,kind,strike,qty,abs(avg),name,symbol); legs.append(leg)
        key=name if name else None
        if key: strategies.setdefault(key, Strategy(key,[],True,"current")).legs.append(leg)
        else: strategies[f"{tk} {expiry} {kind}{strike:g} Single"]=Strategy(f"{tk} {expiry} {kind}{strike:g} Single",[leg],True,"current")
    return list(strategies.values()), legs

def price_grid(center:float,pct:float=0.5,steps:int=201)->np.ndarray:
    return np.linspace(max(0.0,center*(1-pct)), center*(1+pct), steps)
...
def aggregate_pl_curves(strategies:List[Strategy], xs:np.ndarray, ticker:Optional[str]=None)->np.ndarray:
    tot=np.zeros_like(xs,float)
    for s in strategies:
        if not s.enabled: continue
        if ticker:
            if ticker in s.tickers(): tot+=strategy_pl_curve(s,xs,ticker)
        else:
            for tk in s.tickers():
                tot+=np.array([sum(l.payoff_at_expiry(x) for l in s.legs if l.ticker==tk) for x in xs])
    return tot
def breakevens(xs:np.ndarray, ys:np.ndarray)->List[float]:
    idx=np.where(np.diff(np.sign(ys))!=0)[0]; out=[]
    for i in idx:
        x0,x1=xs[i],xs[i+1]; y0,y1=ys[i],ys[i+1]
        if y1!=y0: out.append(round(float(x0 - y0*(x1-x0)/(y1-y0)),4))
    return out

def build_single(t,exp,k,strike,qty,price,tag="what-if")->Strategy:
    return Strategy(f"{t} {exp} {('Long' if qty>0 else 'Short')}{k} {strike:g}",[OptionLeg(t,exp,k,strike,qty,abs(price))],True,tag)
def build_vertical(t,exp,k,short_k,long_k,qty,credit,tag="what-if")->Strategy:
    return Strategy(f"{t} {exp} {k} Vertical {long_k:g}/{short_k:g}",[
        ...
        OptionLeg(t,exp,k,long_k,+abs(qty),0.0)],True,tag)
def build_iron_condor(t,exp,sp,lp,sc,lc,qty,credit,tag="what-if")->Strategy:
    return Strategy(f"{t} {exp} Iron Condor P:{lp:g}/{sp:g} C:{sc:g}/{lc:g}",[
        ...
        OptionLeg(t,exp,"C",lc,+abs(qty),0.0)],True,tag)
def build_strangle(t,exp,pk,ck,qty,credit,tag="what-if")->Strategy:
    ...
    return Strategy(f"{t} {exp} {'Short' if short else 'Long'} Strangle {pk:g}/{ck:g}",[
        ...
        OptionLeg(t,exp,"C",ck,-abs(qty) if short else +abs(qty),abs(credit)/2.0)],True,tag)
def build_iron_butterfly(t,exp,center_k,put_wing,call_wing,qty,credit,tag="what-if")->Strategy:
    return Strategy(f"{t} {exp} Iron Butterfly {put_wing:g}-{center_k:g}-{call_wing:g}",[
        ...
        OptionLeg(t,exp,"C",call_wing,+abs(qty),0.0)],True,tag)

class QuoteProvider:
    Yahoo="Yahoo Finance (yfinance)"; Tradier="Tradier"; Polygon="Polygon.io"; Moomoo="Moomoo (custom stub)"
def normalize_symbol_for_yahoo(t:str)->str: return t.strip().upper().replace(" ","").replace(".","-")
def _safe_float(v):
def get_available_strikes(provider: str, ticker: str, expiry: dt.date, api_key: str = "", moomoo_cookie: str = "") -> List[float]:
    """Return sorted list of available strikes for the nearest expiry to `expiry` (provider dependent)."""
    strikes: List[float] = []
    try:
        if provider == QuoteProvider.Yahoo and yf:
            tk = normalize_symbol_for_yahoo(ticker)
            t = yf.Ticker(tk)
            # choose nearest available expiry
            exps = t.options or []
            if not exps:
                return strikes
            def to_date(s):
            ...
            calls = chain.calls if chain and getattr(chain, "calls", None) is not None else None
            puts  = chain.puts  if chain and getattr(chain, "puts", None)  is not None else None
            if calls is not None and not calls.empty:
                strikes.extend(list(calls["strike"].astype(float).unique()))
            if puts is not None and not puts.empty:
                strikes.extend(list(puts["strike"].astype(float).unique()))
...
if st:
    ...
    def yahoo_available_expiries(ticker:str)->List[str]:
        try: return list((yf.Ticker(normalize_symbol_for_yahoo(ticker)).options) or [])
    ...
    def fetch_spot(provider:str,ticker:str,api_key:str="",moomoo_cookie:str="")->Optional[float]:
        try:
            if provider==QuoteProvider.Yahoo and yf:
                tk=normalize_symbol_for_yahoo(ticker); t=yf.Ticker(tk)
                try:
                    fi=t.fast_info
                ...
                try:
                    info=t.info or {}; v=_safe_float(info.get("regularMarketPrice") or info.get("currentPrice"))
                    if v and v>0: return v
                except Exception: pass
                try:
                ...
                try:
                    d=yf.download(tk, period="1d", interval="1m", progress=False, threads=False)
            ...
            if provider==QuoteProvider.Polygon and requests:
                r=requests.get(f"https://api.polygon.io/v2/last/trade/{ticker}", params={"apiKey":api_key}, timeout=8); r.raise_for_status()
                return float(r.json()["results"]["p"])
            return None
        except Exception as e:
            st.warning(f"Spot fetch error for {ticker}: {e}"); return None
    ...
    def fetch_option_mid(provider:str,ticker:str,expiry:dt.date,kind:str,strike:float,api_key:str="",moomoo_cookie:str="")->Optional[float]:
        try:
            if provider==QuoteProvider.Yahoo and yf:
                ...
                except Exception as e: st.warning(f"Option chain fetch error {tk} {exp}: {e}"); return None
                tbl=chain.calls if kind=="C" else chain.puts
                if tbl is None or tbl.empty: return None
            ...
            st.warning(f"Option mid fetch error {ticker} {expiry} {kind}{strike}: {e}"); return None
...
def st_app():
    ...
    elif use_sample: df=sample_positions_df()
    else: st.warning("Upload a CSV or load the sample to begin."); st.stop()

    try: strategies, legs = parse_moomoo_positions(df)
    except Exception as e: st.error(f"Failed to parse CSV: {e}"); st.dataframe(df.head(50)); st.stop()
    ...
    for tk in tickers:
        if f"spot_{tk}" not in st.session_state:
            ks=[l.strike for l in legs if l.ticker==tk]; st.session_state[f"spot_{tk}"]=float(np.median(ks)) if ks else 500.0
        ...
        if st.sidebar.button(f"Fetch {tk} spot", key=f"btn_spot_{tk}"):
            ...
            if fetched: st.session_state[f"spot_{tk}"]=float(fetched); st.experimental_rerun()
            else: st.warning(f"{tk}: could not fetch spot from {provider}. Set manually or try another provider.")
    ...
    for s in strategies: s.enabled=st.checkbox(s.name, True, key=f"cur_{s.name}")
    ...
    with st.expander("Builder"):
        ...
        with tabs[3]:
            ...
            if st.button("🔧 Snap to market", key="ib_snap"): 
                ...
                if strikes:
                    # center -> nearest listed strike
                    nearest = float(sorted(strikes, key=lambda s: abs(s - center))[0])
                    st.session_state["ib_center"] = nearest
                    # width -> nearest multiple of chosen step
                    sc = float(st.session_state.get("ib_step", 5.0))
                    snapped_w = round(max(0.0, width) / sc) * sc
                    if snapped_w <= 0 and sc > 0:
                        snapped_w = sc
                    st.session_state["ib_width"] = float(snapped_w)
                    st.experimental_rerun()
                else:
                    st.warning("No strikes from provider; manual values kept.")
            put_wing=float(max(0.0, center-width)); call_wing=float(center+width)
            ...
            if st.button("Auto price from API", key="ib_auto"):
                pm =fetch_option_mid(provider, tk, exp, "P", center,   api_key=(tradier_token or polygon_key), moomoo_cookie=moomoo_cookie)
                cm =fetch_option_mid(provider, tk, exp, "C", center,   api_key=(tradier_token or polygon_key), moomoo_cookie=moomoo_cookie)
            ...
            if st.button("➕ Add Iron Butterfly"):
                what_if.append(build_iron_butterfly(tk,exp,float(center),float(put_wing),float(call_wing),int(qty),float(st.session_state[credit_key]))); st.success("Added Iron Butterfly")
    ...
    if what_if:
        for i,s in enumerate(what_if):
            s.enabled=st.checkbox(s.name, True, key=f"wi_{i}")
            c1,c2=st.columns([2,1])
            with c1: st.text(s.describe())
            with c2:
                if st.button("❌ Remove", key=f"rm_{i}"): what_if.pop(i); st.experimental_rerun()

    st.checkbox("Show diagnostics", value=False, key="diag")

    st.header("P/L at Expiry (per Ticker)")
    for tk in tickers:
        xs=price_grid(spot_inputs[tk], pct, steps)
        cur=sum(strategy_pl_curve(s,xs,tk) for s in strategies if s.enabled and tk in s.tickers())
        add=sum(strategy_pl_curve(s,xs,tk) for s in what_if if s.enabled and tk in s.tickers())
        comb=cur+add
        fig=go.Figure()
        fig.add_trace(go.Scatter(x=xs,y=cur,name="Current"))
        fig.add_trace(go.Scatter(x=xs,y=comb,name="Current + What-If"))
        fig.add_hline(y=0, line_dash="dash"); fig.add_vline(x=spot_inputs[tk], line_dash="dot")
        fig.update_layout(height=400, xaxis_title="Underlying at Expiry", yaxis_title="P/L (USD)")
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Current**"); st.write({"P/L @ spot": round(float(np.interp(spot_inputs[tk], xs, cur)),2),
                                              "Max": round(float(np.max(cur)),2), "Min": round(float(np.min(cur)),2),
                                              "Breakevens": breakevens(xs,cur)})
        st.markdown("**Current + What-If**"); st.write({"P/L @ spot": round(float(np.interp(spot_inputs[tk], xs, comb)),2),
                                                        "Max": round(float(np.max(comb)),2), "Min": round(float(np.min(comb)),2),
                                                        "Breakevens": breakevens(xs,comb)})
        if st.session_state.get("diag"):
            st.write("Legs:"); 
            for s in strategies+what_if:
                for lg in s.legs:
                    if lg.ticker==tk: st.write(lg.label())

    st.header("Portfolio (Combined) – Approximate")
    tk0=tickers[0]; xs=price_grid(spot_inputs[tk0], pct, steps)
    current=aggregate_pl_curves(strategies, xs, ticker=tk0); added=aggregate_pl_curves(what_if, xs, ticker=tk0)
    comb=current+added
    fig2=go.Figure()
    fig2.add_trace(go.Scatter(x=xs,y=current,name="Current"))
    fig2.add_trace(go.Scatter(x=xs,y=comb,name="Current + What-If"))
    fig2.add_hline(y=0, line_dash="dash"); fig2.add_vline(x=spot_inputs[tk0], line_dash="dot")
...
def _test_payoff_basic():
    ...
    leg=OptionLeg("ABC", dt.date(2025,1,1), "P", 50, -2, 1.5)
...
def _test_parser_and_grouping():
    df=sample_positions_df(); strategies,legs=parse_moomoo_positions(df)
    strikes=sorted({l.strike for l in legs}); assert strikes==[195.0,200.0]

def _run_tests():
    _test_payoff_basic(); _test_parser_and_grouping(); print("All tests passed.")

def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--run-tests", action="store_true")
    args,_=parser.parse_known_args()
    if args.run_tests: _run_tests(); return
    if st is None:
        print("Install deps: pip install streamlit plotly pandas numpy yfinance requests\nRun: streamlit run option_pl_simulator.py"); return
    st_app()
